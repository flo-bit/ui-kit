<script lang="ts">
	...
	onMount(() => {
		...
		const sync = () => {
			...
			document.documentElement.dataset.enhance = true;
	...
	class Slider {
		constructor(element) {
			...
			if (!CSS.supports('animation-timeline: scroll()')) {
				...
				input.addEventListener('pointerdown', ({ y }) => {
					const { top, height } = input.getBoundingClientRect();
					const range = input.max - input.min;
					const val = range - Math.floor(range * ((y - top) / height));
					input.value = val;
					sync();
				});
				sync();
			...
			Draggable.create(PROXY, {
				...
				onDrag: function () {
					...
					if (this.y < start * -1) {
						...
						wrap.style.setProperty(
							'--stretch',
							gsap.utils.mapRange(0, config.stretch, 0, 1, Math.abs(start * -1 - this.y))
						);
					} else if (this.y > end) {
	...
	<div class="container">
		<div class="slider-wrap">
			...
			<div class="slider slider--brightness">
				...
				<input tabindex="0" id="brightness" type="range" min="0" max="100" value="86" />
	...
	<div class="container">
		<div class="slider-wrap">
			...
			<div class="slider slider--volume">
				...
				<input tabindex="0" id="volume" type="range" min="0" max="100" value="50" />
				<div class="slider__indicator">
					<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
						<path
	...
	<div class="container">
		<div class="slider-wrap">
			...
			<div class="slider slider--intensity">
				...
				<input tabindex="0" id="intensity" type="range" min="0" max="100" value="0" />
				<div class="slider__indicator">
...
<style>
	* {
	...
	[data-enhance='true'] :is(.track) {
		scale: calc(1 - (clamp(0, var(--stretch), 1) * (var(--stretch-ratio, 0.2) * 0.5)))
	...
	.slider__track {
		...
		--perfect-shadow:
			0px 0px 0px 1px hsl(0 0% 0%), -9px 9px 9px -0.5px hsl(0 0% 0% / 0.5),
			-18px 18px 18px -1.5px hsl(0 0% 0% / 0.5), -37px 37px 37px -3px hsl(0 0% 0% / 0.5),
			-75px 75px 75px -6px hsl(0 0% 0% / 0.5), -150px 150px 150px -12px hsl(0 0% 0% / 0.5);
