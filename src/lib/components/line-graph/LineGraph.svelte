	...
	<div class="h-44 w-full overflow-visible p-4">
		...
		>
			<Svg>
				...
				/>
				<Spline class="stroke-4 [stroke-linecap:round] stroke-accent-500 fill-transparent" curve={curveCatmullRom} />
				<Highlight lines points={{ class: 'fill-transparent outline-accent-700 dark:outline-accent-200 outline-4 rounded-full' }} />
			</Svg>

			<Tooltip.Root let:data class="border-base-300 bg-base-200/80 dark:border-base-800 dark:bg-base-900/80 rounded-2xl border py-2 backdrop-blur-xs">
				...
				<Tooltip.List>
					<Tooltip.Item
						label=""
						value={data.value}
						class="text-accent-500 font-bold flex gap-2"
					/>
				</Tooltip.List>
			</Tooltip.Root>
		</Chart>
