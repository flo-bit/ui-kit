<script lang="ts" module>
	export const blueskyLoginModalState = $state({
		visible: false,
		show: () => (blueskyLoginModalState.visible = true),
		hide: () => (blueskyLoginModalState.visible = false)
	});
</script>
...
<script lang="ts">
	...
	$effect(() => {
		if (!blueskyLoginModalState.visible) {
			error = null;
			value = '';
			loading = false;
		} else {
...
<Modal
	bind:open={blueskyLoginModalState.visible}
...
>
	...
	<form onsubmit={onSubmit} class="flex flex-col gap-2">
		...
		{#if error}
			<p class="mt-2 text-accent-500 text-sm">{error}</p>
		{/if}
