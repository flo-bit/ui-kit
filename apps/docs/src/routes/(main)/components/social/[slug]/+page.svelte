<script lang="ts">
	...
	const modules = import.meta.glob('/src/lib/docs/social/*/index.ts', { eager: true }) as Record<string, ComponentDoc>;

	let slug = $derived(page.params.slug);

	let componentDoc = $derived.by(() => {
		const mod = modules[`/src/lib/docs/social/${slug}/index.ts`];
