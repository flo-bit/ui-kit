<script lang="ts">
	...
	let { items, range, editor, active = 0 }: Props = $props();
...
>
	{#each items as item, index (item.value)}
		<button
			...
			class={cn(
				'text-base-900 dark:text-base-200 group relative isolate flex w-full min-w-28 cursor-pointer items-center gap-3 px-3 py-2 font-medium [&_svg]:size-3.5',
				activeIndex === index
